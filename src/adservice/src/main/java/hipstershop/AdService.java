  ...
  private void start() throws IOException {
    ...
    server =
        ServerBuilder.forPort(port)
            ...
            .build()
            .start();
    logger.info("Ad Service started, listening on " + port + "   Server: " + server + "    class=", server.getClass());
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread(
                () -> {
                  ...
                  System.err.println(
                      "*** shutting down gRPC ads server since JVM is AAA shutting down");
                  AdService.this.stop();
...
//    RpcViews.registerAllGrpcViews();
    ...
    new Thread(
            () -> {
              try {
                logger.info("BBB main 2.1");
                initStats();
                logger.info("BBB main 2.2");
                initTracing();
                logger.info("BBB main 2.3");
              } catch (Throwable t) {
                logger.info("BBB main 2.4");
                logger.info("BBB main 2.4 ", t);
              }
            })
        .start();

    // Register Jaeger
    logger.info("BBB main 3");
    initJaeger();
    logger.info("BBB main 4");

    // Start the RPC server. You shouldn't see any output from gRPC before this.
    logger.info("AdService starting.");
    final AdService service = AdService.getInstance();
    logger.info("BBB main 5");
    service.start();
    logger.info("BBB main 6");
    service.blockUntilShutdown();
    logger.info("BBB main 7");
  }
}
